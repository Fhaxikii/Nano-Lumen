...
用法：
  py -3.10 tests\t_f1_stage7_guitask.py
"""
...
def t_owner_needs_own_expiry(tmp: pathlib.Path) -> None:
    ...
    **那个目标达成或放弃** —— 而那**只有模型知道**。
    ...
    # ── 判据留痕 ────────────────────────────────────────────────────────
    # ⚠️ 出处**从内部文档换成了随仓库发行的 `docs/`**。
    #    原先指向 `todo/` 下那份早先的设计 —— 它不随仓库分发，于是克隆下来的人
    #    一跑就是红的，而读者无从判断这红是不是自己弄出来的。
    # 📌 但这条断言存在的理由没变：**一条断言必须能指回它的来源**，
    #    否则它会在某次重构里被当成「不知道为什么存在的规矩」删掉。
    #    ⇒ 所以正确的修法是**把来源搬到读者看得到的地方**，而不是把断言删掉。
    _doc = (ROOT / "docs" / "zh" / "02-architecture.md").read_text(encoding="utf-8")
    check("都不许把 Task 的结束当成它唯一的失效条件" in _doc,
          "⚠️ 判据写在 docs/zh/02-architecture.md —— 下一个加「绑 Task」的人会看到")
    check("而模型会忘" in _doc,
          "⚠️ 而它的**理由**（Task 的结束依赖模型判断）也一起留了下来")
