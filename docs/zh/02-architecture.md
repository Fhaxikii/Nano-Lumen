...
| `core/models.py` | 厂商与模型的事实表：哪些厂商可用、每个厂商有哪些模型、各自支持什么；三个内部角色槽（压缩/命令审查/视觉）也在这里解析 |
...
| `core/tools/builtin.py` | 全部内置工具的唯一一处声明（manifest / 感知 / 工具卡 / 调度 / 执行） |
...
| `core/mcp_discovery.py` | MCP 发现链的第一环：检索官方 MCP Registry（名/版本/包/repo/安装方式） |
| `core/reading.py` | 迭代阅读：大文件先试读定位、再分片深读、读完记结论丢原文 |
| `core/code_scan.py` | 代码副作用扫描（Skill 审计与临时执行通道共用的唯一判据） |
...
| `executor_write.py` | 系统 API 写动作：窗口/音量/进程/文件读写/剪贴板/打开 URL（不含鼠标键盘） |
| `executor_action.py` | 鼠标键盘动作 + 双保险急停（全局 ESC 监听 + 屏幕角 failsafe） |
| `executor_vision.py` | 视觉定位器：UIA 控件树优先，拿不到时降级到多模态视觉模型 |
| `longcmd.py` | 长命令后台化：命令活过工具调用，stdout 可回看 |
| `cmd_classifier.py` | Auto 模式的危险命令判定（对齐当前任务意图） |
| `window_binding.py` | 当前操作窗口绑定：Nano 知道自己在操作哪个窗口，防清掉用户的文件 |
| `fileedit.py` | `edit_file` 的纯计算部分：算 diff，不直接写盘 |
| `filesearch.py` | `search_files`：文件名通配 + 内容 grep，合成一个只读工具 |
| `pathpolicy.py` | 文件工具的路径黑名单（SAM、私钥、凭据目录等） |
| `safety.py` | 授权作用域与步数计数 |
| `audit.py` | 全量 OS 操作流水（append-only JSON，只读动作也留痕） |
| `canary.py` | 空闲时用任务栏标准按钮自测视觉定位链路，退化主动告警 |

### 存储与记忆层

这一层最容易被目录名误导，先说结论：**`memory/` 不是存储层**，它只是当前对话在内存里的投影。真正落盘的东西分散在下面三处。

| 文件 / 目录 | 职责 |
|---|---|
| `memory/manager.py` | **当前对话的内存消息投影**：ChatMessage 列表、按交换截断、图片压缩、系统注记。重置对话即清空；落盘是委托给 `core/runtime/` 的 |
| `core/runtime/conversation.py` | 对话原文的权威账本（用户真正说过什么） |
| `core/runtime/store.py` | Runtime 的 SQLite 持久层（`data/nano_runtime.db`），四张表：tasks / runtime_actions / commands / interactions |
| `core/memory_store.py` | Working Memory：跨会话持久的操作记录（`data/nano_memory.db`），模型可主动 recall |
| `core/semantic_memory.py` | 语义记忆的写入触发与写前提纯（什么时候写、写之前怎么脱敏；存储在另外两个文件） |
| `core/memory_index.py` | 语义记忆的向量召回（独立 collection，与知识库不混用） |
| `core/runtime/blobs.py` | 用户发过图片的内容寻址图库（上下文可以忘，历史不能忘） |
...
| 文件 | 职责 |
|---|---|
| `task.py` | Task 脊柱：跨轮、可挂起、可后台化的状态归属单位（不做任务智能编排） |
| `kernel.py` | Runtime 唯一写入口：所有状态变更走 `submit()` 一处，固定做校验与幂等 |
| `reconciler.py` | Level-triggered 收敛：每次重读当前状态推导该做什么，不靠边沿通知 |
| `waitcond.py` | 「Nano 在等什么」的唯一权威（定时等待、后台完成、用户回答） |
| `interaction.py` | 统一「需要用户回应的事」（审计确认 / 风险确认 / 澄清），一张表一个工具 |
| `inbox.py` | 用户消息永不丢：忙时排队，内核空了自动补发 |
| `outbox.py` | Transactional outbox：状态变更与外部动作之间的崩溃窗口保护 |
| `oslease.py` | 「谁正在操作这台电脑」的租约（OS 操作授权 / 被动挂起） |
| `attempt.py` | 当前这一个动作做到哪了（用户中途接管后恢复时告诉模型上一个动作是否可信） |
| `toolbatch.py` | 工具批次的显式四态生命周期（PREPARED/OPEN/COMMITTED/ABORT） |
| `clock.py` | 可注入统一时钟（测试用 FakeClock，崩溃恢复测试瞬时且确定） |
...
| `export.py` | 导出全部聊天记录（md 给人看，json 不丢字段，图片跟着走） |
...
| 文件 | 职责 |
|---|---|
| `meter.py` | 上下文计量：现在的上下文占窗口多少（锚 + 快照差 + 残差看门狗） |
| `budget.py` | 预算水位：离高水位多远（只观测，不动作） |
| `guard.py` | 硬窗口守卫：启发式底下那条确定性兜底，超限前强制安全降级 |
| `exchange.py` | 「一次交换」——衰减阶梯的单位 |
| `decay.py` | 衰减执行：L0→L1 纯改写、L1→L2 过提炼器 |
| `decay_store.py` | 衰减账本：每段历史现在处于 L 几（与对话账本冲突时以对话账本为准） |
| `digest.py` | L2 结论行的固定 schema 与提炼提示词（本模块不调模型） |
| `bridge.py` | L3 交接：把结论交给语义记忆并验证真的拿得回来，失败就留在 L2 |

### 主动智能（`core/proactive/`）

Beta 阶段整体处于 shadow 观测模式，默认不主动开口，但代码是完整的。

| 文件 / 目录 | 职责 |
|---|---|
| `activity.py` | 实时行为元数据采集（按键节奏、前台窗口、CPU 采样），只记录不判断 |
| `hooks.py` | 系统事件钩子：键盘监听 + 前台窗口轮询 |
| `triggers.py` | 从行为快照生成触发候选（条件是否成立） |
...
| `app_catalog.py` | 应用分类的单一事实来源（进程名 → 类别） |
...
| `core/crash_journal.py` | 崩溃留痕：危险操作先写 breadcrumb，进程死了重启能定位死因（抓 segfault 这类钩子抓不到的） |
