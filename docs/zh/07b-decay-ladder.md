...
| 箭头 | 函数 | 调模型？ | 驱动条件 | 产物 |
|---|---|---|---|---|
| L0→L1 | `run_l0_to_l1`（:215） | 否 | L0 占用降到目标以下即停，或本轮降满 `MAX_PER_RUN` | 工具结果正文 → 占位符 |
| L1→L2 | `run_l1_to_l2`（:404） | **是，每次一个调用** | L1 占用超过该模型配额（`_quota.L1` × 窗口） | 结论行（Digest） |
| L2→L3 | `run_l2_to_l3`（:560） | 否（结论行已在库里） | 达到 L3 条件的交换 | 语义记忆 + 索引条目 |
| L3→L4 | `run_l3_to_l4`（:681） | 否 | 索引条目过期 | 索引条目删除（**不删语义记忆**） |
...
- **L1 是「替换」，不是「第二次截断」**。已有的单条安全阀
  ...
  到 L1 都整个换占位符。
- **`MAX_PER_RUN = 40`（decay.py:54）不是性能考虑，是防失控**：
