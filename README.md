...
`常驻型智能体` · `跨会话持久状态` · `本地优先隐私` · `Windows 桌面端`

---

会话会结束，进程会崩溃，机器会重启。Nano 的不同之处是——**它不会从零开始。**

Nano 不是又一个 Agent 外壳。它把智能体当成一台**长期存在于用户电脑里、拥有持久状态的系统**来设计：状态跨会话连续，崩溃可恢复，记忆不归零。

---

## 🔄 核心范式差异：会话式 vs 连续性

当前主流 Agent 产品普遍采用**会话式**设计：

- 一次会话结束，工作态、上下文、记忆清零
- 新建会话时，需要重新交代背景与前置信息

Nano 是**连续性**的，从底层定义了 Agent 的持久状态属性：

- 状态跨会话持久、可恢复
- 进程崩溃、重启、断电后能自动恢复，并向用户如实说明「那个任务要不要继续」
- 跨会话记得用户让它找过的文件、记过的偏好
...
- 三层主动引擎（硬安全 / 常规触发 / 状态推断）+ 情绪模型已建成；当前处于 **shadow 观测期**：只记录决策、不真正开口，不会在你未要求时突然发消息。情绪只影响语气与主动闸门，不触碰硬功能

**🎛️ 系统操作与安全**

- **操作管控**：系统操作按风险分级，六个独立权限开关 + 带分类器的 Auto 模式；Auto 下以「动作是否符合你的意图」判定危险命令并拦截，与意图不符的行为被挡住
- **操作轨迹感知**：仅记录「在哪个应用、在做什么」级别的环境摘要（约 18 小时过期），行为账本使用封闭类别、不含敏感语义，默认无遥测
- **子智能体**：可派生受限作用域的分身，并行探索或执行任务，工具与运行范围经白名单隔离
- **原生窗口**：基于 NiceGUI + WebView2 构建的原生窗口化交互界面
- **日志 + UIA 自检**：所有 OS 动作日志留痕，空闲时自动验证视觉定位链路是否正常
