...
![Python](https://img.shields.io/badge/Python-3.10-3776AB) ![Platform](https://img.shields.io/badge/Platform-Windows%20Desktop-0078D6) ![License](https://img.shields.io/badge/License-Apache--2.0-brightgreen) ![Release](https://img.shields.io/github/v/release/Fhaxikii/Nano-Lumen) ![Stars](https://img.shields.io/github/stars/Fhaxikii/Nano-Lumen?style=social)

![Nano-Lumen](assets/nano-banner.png)

**Nano-Lumen v1.96** · Windows 桌面常驻型通用智能体 · [English](README.en.md)

`常驻型智能体` · `任务级持久状态` · `本地优先隐私` · `Windows 桌面端`

---

进程会崩溃，机器会重启。Nano 的不同之处是——**它不会从零开始。**

Nano 不是又一个 Agent 外壳。它把智能体当成一台**长期存在于用户电脑里、拥有持久状态的系统**来设计：状态跨进程连续，任务可恢复，一次崩溃不等于一切归零。

---

## 🔄 从「会话」到「常驻系统」：Nano 为什么没有 session

主流 Agent 以「会话」为生命周期单位：上下文归属在某次会话里，再靠项目记忆、历史文件等手段在会话之间延续。作为设计这是自洽的——它默认 Agent 是一个「被打开、用完、关掉」的工具。

Nano 的出发点不同：它常驻在桌面，是一份长期存在的连续运行。对一个长期存在的智能体来说，用一堆互不相通的「会话」与用户相处本身就是别扭的——你不会每次跟同一个人说话都先「新建会话」。

所以 Nano 去掉了 session 这个概念，改用 **Task** 作为状态归属单位：

- **Task 是一件工作的归属**：它的上下文、授权、后台子资源与成本记账都挂在 Task 上；Task 可以跨多轮对话、被挂起、被插队，不随某次「会话结束」而消失
- **对话原文是一条持久账本**：写入即落盘（SQLite），直到你显式重置；上下文的连续性靠持久化与分层压缩解决，而不依赖「你正处在哪个会话里」
- **崩溃、断电、重启之后**，未完成的 Task 如实恢复，并询问「要不要继续」——而不是默认从零开始

这不是「记得比别人久」，而是生命周期单位的不同：**Agent 不是一串会话，而是一个持续存在的系统。**
...
└──────────────┬──────────────┘
┌──────────────▼──────────────┐
│ 编排层 core/orchestrator.py │  ReAct 主循环
└──┬──────────┬──────────┬────┘
   │          │          │
┌──▼───┐ ┌───▼────┐ ┌───▼─────┐
│模型层 │ │工具层   │ │存储记忆层 │
...
名字来自 GNU nano——一个经典的终端编辑器。对我们而言，nano 是很多人第一次写 Python 时用的编辑器。

在 GUI 普及之前，人要学会机器的语言才能操作计算机；GUI 的出现，让人类不必再记忆命令行。

我们认为的下一代交互范式：**机器学会听人的话。**

这不是一个完成的产品，而是一段很长的路的开始。
