...
- **按需加载**：工具动态感知、按需注入，压低每轮开销

**🧠 记忆与上下文**

- **对话原文持久化**（SQLite），重启不丢，长对话自动按 L0→L4 分层衰减压缩，控制成本
...
- 主动行为引擎已建成，但当前处于 **shadow 观测期**：只记录决策、不真正开口，不会在用户未要求时突然发消息

**🎛️ 系统操作与安全**

- **操作管控**：系统操作按风险分级管控，提供六个独立权限开关，支持带分类器的 Auto 模式，拦截与用户意图不符的危险行为
